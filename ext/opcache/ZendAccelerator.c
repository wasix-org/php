...
static zend_persistent_script *opcache_compile_file(zend_file_handle *file_handle, int type, zend_op_array **op_array_p)
{
	...
	*/
	...
	new_persistent_script->script.first_early_binding_opline =
		(op_array->fn_flags & ZEND_ACC_EARLY_BINDING) ? zend_build_delayed_early_binding_list(op_array) : (uint32_t)-1;
...
zend_op_array *persistent_compile_file(zend_file_handle *file_handle, int type)
{
	...
	if (!file_handle->filename || !ZCG(accelerator_enabled))
	{
		...
		if (file_handle->filename && ZCG(accel_directives).file_cache && ZCG(enabled) && accel_startup_ok)
		{
	...
	else
	{
		...
		if (!persistent_script)
		{
			...
			if (file_handle->type == ZEND_HANDLE_FILENAME && accelerator_orig_zend_stream_open_function(file_handle) == FAILURE)
			{
				if (!EG(exception))
				{
					if (type == ZEND_REQUIRE)
...
static zend_result accel_post_startup(void)
{
	...
	if (!file_cache_only)
	{
		...
		if (JIT_G(enabled) && JIT_G(buffer_size) && zend_jit_check_support() == SUCCESS)
		{
			size_t page_size;
		...
		if (JIT_G(enabled))
		{
			if (JIT_G(buffer_size) == 0 || !ZSMMG(reserved) || zend_jit_startup(ZSMMG(reserved), jit_size, reattached) != SUCCESS)
			{
				JIT_G(enabled) = 0;
				JIT_G(on) = 0;
...
static size_t preload_try_strip_filename(zend_string *filename)
{
	...
	if (ZSTR_LEN(filename) > sizeof(" eval()'d code") && *(ZSTR_VAL(filename) + ZSTR_LEN(filename) - sizeof(" eval()'d code")) == ':')
	{
...
static void preload_link(void)
{
	...
	do
	{
		...
		ZEND_HASH_FOREACH_STR_KEY_VAL_FROM(EG(class_table), key, zv, EG(persistent_classes_count))
		{
			...
			if ((ce->ce_flags & (ZEND_ACC_TOP_LEVEL | ZEND_ACC_ANON_CLASS)) && !(ce->ce_flags & ZEND_ACC_LINKED))
			{
		...
		EG(class_table), key, zv, EG(persistent_classes_count))
	{
		...
		if ((ce->ce_flags & (ZEND_ACC_TOP_LEVEL | ZEND_ACC_ANON_CLASS)) && !(ce->ce_flags & ZEND_ACC_LINKED))
		{
			zend_string *lcname = zend_string_tolower(ce->name);
			preload_error error;
			if (!(ce->ce_flags & ZEND_ACC_ANON_CLASS) && zend_hash_exists(EG(class_table), lcname))
			{
				zend_error_at(
...
static int accel_preload(const char *config, bool in_child)
{
	...
	if (ret == SUCCESS)
	{
		...
		ZEND_HASH_FOREACH_PTR(preload_scripts, script)
		{
			...
			ZCSG(saved_scripts)
			[i++] = preload_script_in_shared_memory(script);
		}
		ZEND_HASH_FOREACH_END();
		ZCSG(saved_scripts)
		[i] = NULL;
...
static int accel_finish_startup(void)
{
	...
	if (ZCG(accel_directives).preload && *ZCG(accel_directives).preload)
	{
		...
		if (geteuid() == 0)
		{
			...
			if (!ZCG(accel_directives).preload_user || !*ZCG(accel_directives).preload_user)
			{
				zend_shared_alloc_unlock();
